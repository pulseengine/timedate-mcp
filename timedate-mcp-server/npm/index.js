...
function getPlatformPackageName() {
  ...
  return `@pulseengine/timedate-mcp-server-${platformName}-${archName}`;
}
...
function getBinaryPath() {
  const platform = os.platform();
  const binaryName = platform === "win32" ? "timedate-mcp-server.exe" : "timedate-mcp-server";
...
function getPlatformInfo() {
  return {
    ...
    binaryName: os.platform() === "win32" ? "timedate-mcp-server.exe" : "timedate-mcp-server"
  };
}
