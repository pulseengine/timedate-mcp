...
function getPlatform() {
  ...
  if (arch === "x64") {
    ...
    throw new Error(`Unsupported architecture: ${arch}. Supported architectures: x64 (Intel), arm64 (Apple Silicon). Please install from source: cargo install --git https://github.com/pulseengine/timedate-mcp.git timedate-mcp-server`);
  }
...
function getBinaryName() {
  ...
  return platform === "Windows_NT" ? "timedate-mcp-server.exe" : "timedate-mcp-server";
}
...
function getDownloadUrl() {
  ...
  return `https://github.com/pulseengine/timedate-mcp/releases/download/v${version}/timedate-mcp-server-v${version}-${platform}.${archiveExtension}`;
}
...
async function installBinary() {
  ...
  // Download archive
  const archiveExtension = os.type() === "Windows_NT" ? "zip" : "tar.gz";
  const archivePath = path.join(binDir, `timedate-mcp-server.${archiveExtension}`);
...
installBinary().catch(err => {
  console.error("❌ Failed to install timedate-mcp-server binary:", err.message);

  // Provide helpful error message
  console.error("\n📋 Installation failed. You can:");
  console.error("1. Install Rust and build from source:");
  console.error("   git clone https://github.com/pulseengine/timedate-mcp.git");
  console.error("   cd timedate-mcp/timedate-mcp-server");
  console.error("   cargo install --path .");
  console.error("");
  console.error("2. Download binary manually from:");
  console.error("   https://github.com/pulseengine/timedate-mcp/releases");
